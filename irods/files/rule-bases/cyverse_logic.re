...
#
# Parameters:
#  DataObjPath  the absolute path to the data object
#  RescHier     the resource hierarchy of the replica to replicate
#
cyverse_logic_schedChksumRepl(*DataObjPath, *RescHier) {
	*dataObjId = _cyverse_logic_getDataObjId(*DataObjPath);

	*replNum = 0;
	foreach ( *rec in
		SELECT DATA_REPL_NUM WHERE DATA_ID == *dataObjId AND DATA_RESC_HIER == *RescHier
	) {
...
_cyverse_logic_mkAVUObj(*Attr, *Val, *Unit) = cyverse_json_obj(
	list(
		('attribute', cyverse_json_str(*Name)),
		('value', cyverse_json_str(*Value)),
		('unit', cyverse_json_str(*Unit)) ) )

_cyverse_logic_mkUserObj(*Name, *Zone) = cyverse_json_obj(
	list(('name', cyverse_json_str(*Name)), ('zone', cyverse_json_str(*Zone))) )
...
_cyverse_logic_mkPathField(*Path) = ('path', cyverse_json_str("*Path"))
...
_cyverse_logic_sendAVUAddWildcard(
	*DataObjPat, *AttrName, *AttrVal, *AttrUnit, *AuthorName, *AuthorZone
) {
	*msg = cyverse_json_obj(
		list(
			_cyverse_logic_mkAuthorField(*AuthorName, *AuthorZone),
			('pattern', cyverse_json_str(*DataObjPat)),
...
_cyverse_logic_sendEntityMv(*Type, *Id, *OldPath, *NewPath, *AuthorName, *AuthorZone) {
	*msg = cyverse_json_obj(
		list(
			_cyverse_logic_mkAuthorField(*AuthorName, *AuthorZone),
			_cyverse_logic_mkEntityField(*Id),
			('old-path', cyverse_json_str("*OldPath")),
			('new-path', cyverse_json_str("*NewPath")) ) );
...
#
# TODO: rename _cyverse_logic_retrieveDataInfo
...
#
# TODO: rename to cyverse_logic_notifyDataObjCreated
...
# Publishes a message indicating that a data object was created.
#
# TODO: rename to cyverse_logic_notifyDataObjMod
...
# 	_cyverse_logic_registerAction(*id, *me);
# 	*err = errormsg(cyverse_logic_schedChksumRepl(*DataObjInfo.logical_path, 0), *msg);
